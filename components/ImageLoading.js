...
const ImageLoading = forwardRef(({className, style, src, alt, loading, onClick, draggable}, ref) =>
{
    ...
    return (
        <div className={`${className} ${loadedRef.current ? "" : "image-loading"}`} style={style} ref={contRef}>
            <img decoding={loadedRef.current ? "auto" : "async"} draggable={draggable} className={`${className} image-loading-main ${loadedRef.current ? "loaded" : ""}`} onLoad={loaded} style={style} ref={ref} src={src} alt={alt} loading={loading}/>
        </div>
    )
})
